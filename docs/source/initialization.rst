...
.. important::
   ...
   - 无参数初始化有两种形式：值初始化（带有 ``()`` 或 ``{}`` ）和默认初始化（无 ``()`` 或 ``{}`` )。前者会保证进行初始化（调用默认构造，或清零，或混合）；后者只会调用默认构造（如果是平凡的，则什么都不做）。

   - 有参数初始化，可以通过 ``()`` 或者 ``{}`` 的方式进行，两者的差异在于后者更优先匹配初始化列表，以及窄向转换的约束。

   - 在不使用 ``()`` 或者 ``{}`` 的场景下，使用 ``=`` 进行的初始化，属于 **拷贝初始化** 。如果被初始化对象是一个 `class` 类型， **copy构造** 或 **move构造** 会被调用；在使用 ``()`` 或 ``{}`` 的场景下，在 `C++ 17` 之后，除了 ``explicit`` 的约束之外，和直接初始化没有任何语义上的差异。
